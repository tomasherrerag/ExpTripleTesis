...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        ...
        storage="sqlite:///exp2hard.db",
        study_name="exp2_hard",
        load_if_exists=True
    )
    ...
    for trial in study1.trials:
        if trial.state == optuna.trial.TrialState.COMPLETE:
            continue
        objetivo0 = trial.values[0]
        objetivo1 = trial.values[1]

        if (objetivo0 <= 3 and objetivo1 >= 0):
            warm_start.append(trial)


    #ordenar segun puntaje objetivo 0
    warm_start = sorted(warm_start, key=lambda t: t.values[0])

    print(f"Trials seleccionados para warm start: {len(warm_start)}")

    #cargar parametros
    for trial in warm_start:
        study.enqueue_trial(trial.params)


    #optimizar experimento 2
