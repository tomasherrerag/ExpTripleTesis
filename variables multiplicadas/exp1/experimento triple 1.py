...
def simular(tiempoExp, umbralActividad, ruidoInputAprendizaje, ruidoDirectaAprendizaje, ruidoIndirectaAprendizaje, ruidoHiperdirectaAprendizaje, ruidoEntregaAprendizaje, ruidoInputReaccion, ruidoDirectaReaccion, ruidoIndirectaReaccion, ruidoHiperdirectaReaccion, ruidoEntregaReaccion, tauDirectaAprendizaje, tauIndirectaAprendizaje, tauHiperdirectaAprendizaje, TauPostRutaAprendizaje, tauDirectaReaccion, tauIndirectaReaccion, tauHiperdirectaReaccion, TauPostRutaReaccion, dopaAprendizaje, dopaReaccion):
    ...
    Input_neuronsRuidoOriginal = 0.0
    ...
    Input_neurons_tonesRuidoOriginal = 0.0
...
###condiciones primer exp
    if (contadorCoincidencia1Rev > 3 or contadorCoincidencia1Rev == 0 or contadorCoincidencia1NoRev < 97 or contadorCoincidencia1NoRev == 100 or contadorSkipMalo1NoRev > 2 or contadorSkipMalo1Rev > 2 or promedioTiemposNoRev > promedioTiemposRev):
        return {
            "exp1": {
                "noRev":{
...
def calculoScore(metrics):
    ...
    coincidencia1NoRev = metrics["exp1"]["noRev"]["coincidenciaNoRev"]
    skips1NoRev = metrics["exp1"]["noRev"]["skipsNoRev"]
    tiempo1NoRev = metrics["exp1"]["noRev"]["tiempo"]
    ...
    coincidencia1Rev = metrics["exp1"]["rev"]["coincidenciaRev"]
    skips1Rev = metrics["exp1"]["rev"]["skipsRev"]
    tiempo1Rev = metrics["exp1"]["rev"]["tiempo"]
    ...
    score += abs(coincidencia1Rev - 2) ** 2
    score += skips1Rev ** 2
...
def objective(trial):
    ...
    ruidoInputAprendizaje = trial.suggest_float("ruidoInputAprendizaje", 0.0, 4.0)
    ruidoDirectaAprendizaje = trial.suggest_float("ruidoDirectaAprendizaje", 0.8, 40.0)
    ruidoIndirectaAprendizaje = trial.suggest_float("ruidoIndirectaAprendizaje", 0.8, 40.0)
    ruidoHiperdirectaAprendizaje = trial.suggest_float("ruidoHiperdirectaAprendizaje", 0.8, 40.0)
    ruidoEntregaAprendizaje = trial.suggest_float("ruidoEntregaAprendizaje", 0.0, 4.0)
    ruidoInputReaccion = trial.suggest_float("ruidoInputReaccion", 0.0, 4.0)
    ruidoDirectaReaccion = trial.suggest_float("ruidoDirectaReaccion", 0.8, 40.0)
    ruidoIndirectaReaccion = trial.suggest_float("ruidoIndirectaReaccion", 0.8, 40.0)
    ruidoHiperdirectaReaccion = trial.suggest_float("ruidoHiperdirectaReaccion", 0.8, 40.0)
    ruidoEntregaReaccion = trial.suggest_float("ruidoEntregaReaccion", 0.0, 4.0)
    tauDirectaAprendizaje = trial.suggest_float("tauDirectaAprendizaje", 0.8, 40.0)
    tauIndirectaAprendizaje = trial.suggest_float("tauIndirectaAprendizaje", 0.8, 40.0)
    tauHiperdirectaAprendizaje = trial.suggest_float("tauHiperdirectaAprendizaje", 0.8, 40.0)
    TauPostRutaAprendizaje = trial.suggest_int("TauPostRutaAprendizaje", 50, 10000)
    tauDirectaReaccion = trial.suggest_float("tauDirectaReaccion", 0.8, 40.0)
    tauIndirectaReaccion = trial.suggest_float("tauIndirectaReaccion", 0.8, 40.0)
    tauHiperdirectaReaccion = trial.suggest_float("tauHiperdirectaReaccion", 0.8, 40.0)
...
if __name__ == "__main__":
    ...
    for trial in study.trials:
        coinc1NoRev = trial.user_attrs.get("exp1_noRev_coincidencia")
        skips1NoRev = trial.user_attrs.get("exp1_noRev_skips")
        tiempo1NoRev = trial.user_attrs.get("exp1_noRev_tiempo")
        coinc1Rev = trial.user_attrs.get("exp1_rev_coincidencia")
        skips1Rev = trial.user_attrs.get("exp1_rev_skips")
        tiempo1Rev = trial.user_attrs.get("exp1_rev_tiempo")

        if (
            coinc1NoRev is not None
            and coinc1Rev is not None
            and 100 > coinc1NoRev > 96
            and 0 < coinc1Rev < 4
            and skips1NoRev < 2
            and skips1Rev < 2
            and tiempo1Rev > tiempo1NoRev):
